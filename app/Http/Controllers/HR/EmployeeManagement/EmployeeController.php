...
class EmployeeController extends Controller implements HasMiddleware
{
    ...
    public function __construct(EmployeeService $employeeService, BranchService $branchService, CountryService $countryService, UserService $userService, DepartmentService $departmentService, DepartureReasonService $departureReasonService, JobPositionService $jobPositionService)
    {
     ...
     */
    public function store(CreateEmployeeRequest $request, Branch $branch = null)
    {
        $validatedData = $request->validated();
        $employee = $this->employeeService->createEmployee($validatedData, $branch);
        $this->employeeService->sendEmployeeEmailVerification($employee);
        ...
        if ($branch) {
            return Redirect::route('branches.show', ['branch' => $branch])->with($status, $message);
        }
    ...
    public function workInfo(Employee $employee)
    {
        ...
        $user = auth()->user();
        $activeBranches = $this->branchService->getBranches($user, true);
        $departments = $this->departmentService->getDepartments();
        $departureReasons = $this->departureReasonService->getDepartureReasons();
        $responseData = [
            'jobPositions' => $jobPositions,
