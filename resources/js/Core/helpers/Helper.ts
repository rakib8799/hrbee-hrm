...
export const getLogoSrc = (logo: any) => {
    ...
    if(logoPath) {
      ...
      return getAssetPath('media/logos/hrbee-logo-sidebar.svg');
    }
}
...
export const extractDate = (date: Date) => {
    const datePattern = dateFormat?.replace(/g|i|A|h|H|mm|ss|:/g, '')?.trim();
    return moment(date).tz(timezone).format(datePattern);
};
