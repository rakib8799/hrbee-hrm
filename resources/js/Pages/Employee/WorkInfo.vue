        ...
        <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
            ...
            <VForm @submit="submit()" class="form">
                ...
                <div class="card-body border-top p-9">
                    ...
                    <!-- Department, Branch, Manager and Leave Manager Name-->
                    <div>
                        <h3 class="text-dark font-weight-bold mt-7 mb-5">{{ $t('employee.label.approver') }}</h3>

                        <div class="row g-4">
                            <!-- Department Name -->
                            <div class="col-md-6 mb-2 fv-row">
                                <label class="fs-5 fw-semibold mb-2">{{ $t('employee.label.departmentName') }}</label>
                                <Multiselect
                                    :placeholder="$t('employee.placeholder.departmentName')"
                            ...
                            <!-- Branch Name -->
                            <div class="col-md-6 mb-2 fv-row">
                                <label class="required fs-5 fw-semibold mb-2">{{ $t('employee.label.branchName') }}</label>
                                <Multiselect
                                    :placeholder="$t('employee.placeholder.branchName')"
                                    ...
                                    trackBy="text"
                                />
                                <ErrorMessage :errorMessage="formData.errors.branch_id" />
                            </div>

                            <!-- Manager Name -->
                            <div class="col-md-6 mb-2 fv-row">
                                <label class="fs-5 fw-semibold mb-2">{{ $t('employee.label.manager') }}</label>
                                <Multiselect
                                    :placeholder="$t('employee.placeholder.manager')"
                            ...
                            <!-- Leave Manager Name -->
                            <div class="col-md-6 mb-2 fv-row">
                                <label class="fs-5 fw-semibold mb-2">{{ $t('employee.label.leaveManager') }}</label>
                                <Multiselect
                                    :placeholder="$t('employee.placeholder.leaveManager')"
                                    v-model="formData.leave_manager_id"
                                    :searchable="true"
                                    label="text"
                                    trackBy="text"
                                    :options="leaveManagers"
                                />
                            </div>
                        </div>
                    </div>
...
import {getManagers, setLeaveManagers, setManagers} from '@/Core/helpers/EmployeeHelper';
...
const formData = useForm({
    ...
    leave_manager_id: props.employee?.leave_manager_id || '',
...
const activeBranches = ref<Array<any>>([]);
if (Array.isArray(props.activeBranches) && props.activeBranches.length > 0) {
    activeBranches.value = props.activeBranches.map(branch => ({value: branch.id, text: branch.name}));
}

const departments = ref<Array<any>>([]);
if (Array.isArray(props.departments) && props.departments.length > 0) {
    departments.value = props.departments.map(department => ({value: department.id, text: department.name}));
}

const managers = ref<Array<any>>([]);
const leaveManagers = ref<Array<any>>([]);

watch(
    () => formData.branch_id,
    async (newBranchId) => {
        await setManagers(newBranchId, managers, formData, props?.employee);
        await setLeaveManagers(newBranchId, leaveManagers, formData, props?.employee);
    }
);

onMounted(async () => {
    if (formData.branch_id) {
        const response = await getManagers(formData.branch_id);
