                        ...
                        <div class="menu menu-sub menu-sub-dropdown w-600px w-md-625px" data-kt-menu="true">
                            ...
                            <div class="px-7 py-5" data-kt-user-table-filter="form">
                                <div class="row">
                                    ...
                                    <div class="mb-7 col">
                                        <label class="form-label fs-6 fw-semibold">{{ $t('filterOptions.employee.subtitle.label.department') }}:</label>
                                        <Multiselect
                                            :placeholder="$t('filterOptions.employee.subtitle.placeholder.department')"
                                ...
                                <div class="row">
                                    <!-- Start: Branch filter -->
                                    <div class="mb-7 col">
                                        <label class="form-label fs-6 fw-semibold">{{ $t('filterOptions.employee.subtitle.label.branch') }}:</label>
                                        <Multiselect
                                            :placeholder="$t('filterOptions.employee.subtitle.placeholder.branch')"
...
const tableHeader = ref([{
    ...
    {
        columnName: t('commonComponents.employeeComponent.label.status'),
...
if (Array.isArray(props.departments) && props.departments.length > 0) {
    allDepartments.value = props.departments.map(department => ({value: department.id, text:department.name}));
}
...
// assign all the branches from props to allBranches variable.
const allBranches = ref<Array<any>>([]);
if (Array.isArray(props.branches) && props.branches.length > 0) {
    allBranches.value = props.branches.map(branch => ({value: branch.id, text:branch.name}));
}
...
const applyFilter = () => {
    const departmentFilter = selectedDepartment.value;
    const jobPositionFilter = selectedJobPosition.value;
    const branchFilter = selectedBranch.value;
    const statusFilter = selectedStatus.value;

    if (jobPositionFilter.length == 0 && departmentFilter == '' && branchFilter == '' && statusFilter == '') {
        // If nothing is selected, show all employees
        tableData.value = initEmployees.value;
    } else {
        if(jobPositionFilter.length > 0 && departmentFilter != '' && branchFilter != '' && statusFilter != '') {
            // Filter employees based on selected department, job positions, branch and status
            ...
            tableData.value = initEmployees.value.filter(employee => {
                return jobPositionFilter?.some((jobPosition: any) => {
                    return employee.job_positions?.some((employeeJobPosition: any) => {
                        return departmentFilter == employee.department_id && employeeJobPosition == jobPosition && branchFilter == employee.branch_id && employee.is_active == isActive;
                    });
                });
            });
        }
        else if(jobPositionFilter.length > 0 && departmentFilter != '' && branchFilter != '' && statusFilter == '') {
            // Filter employees based on selected department, job positions and branch
            tableData.value = initEmployees.value.filter(employee => {
                return jobPositionFilter?.some((jobPosition: any) => {
                    return employee.job_positions?.some((employeeJobPosition: any) => {
                        return departmentFilter == employee.department_id && employeeJobPosition == jobPosition && branchFilter == employee.branch_id;
                    });
                });
            });
        }
            ...
            tableData.value = initEmployees.value.filter(employee => {
                return jobPositionFilter?.some((jobPosition: any) => {
                    return employee.job_positions?.some((employeeJobPosition: any) => {
                        return departmentFilter == employee.department_id && employeeJobPosition == jobPosition;
                    });
                });
            });
        }
        else if(jobPositionFilter.length == 0 && departmentFilter != '' && branchFilter != '' && statusFilter == '') {
            // Filter employees based on selected department and branch
            tableData.value = initEmployees.value.filter(employee => departmentFilter == employee.department_id && branchFilter == employee.branch_id);
        }
        else if(jobPositionFilter.length > 0 && departmentFilter == '' && branchFilter != '' && statusFilter == '') {
            // Filter employees based on selected job positions and branch
            tableData.value = initEmployees.value.filter(employee => {
                return jobPositionFilter?.some((jobPosition: any) => {
                    return employee.job_positions?.some((employeeJobPosition: any) => {
                        return employeeJobPosition == jobPosition && branchFilter == employee.branch_id;
                    });
                });
            });
        }
            ...
            tableData.value = initEmployees.value.filter(employee => departmentFilter == employee.department_id);
        }
        else if(jobPositionFilter.length > 0 && departmentFilter == '' && branchFilter == '' && statusFilter == '') {
            ...
            tableData.value = initEmployees.value.filter(employee => branchFilter == employee.branch_id);
        }
        else {
