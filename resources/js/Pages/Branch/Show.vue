            ...
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-title m-0" v-show="activeTab === 1">
                    ...
                    <div class="d-flex align-items-center position-relative my-1">
                        <!-- Edit -->
                        <Link v-if="checkPermission('can-edit-branch')" :href="route('branches.edit', props.branch?.id)" class="btn btn-icon btn-flex btn-active-light-primary w-30px h-30px" data-bs-toggle="tooltip" :title="$t('tooltip.title.edit')">
                ...
                <div class="card-title m-0" v-show="activeTab === 2">
                    ...
                    <!--begin::Add Employee-->
                        <Link v-if="checkPermission('can-create-employee')" :href="route('branches.createBranchEmployee', props.branch?.id)" class="btn btn-primary">
            ...
            <div class="card-body pt-0">
                <div id="kt_referred_employees_tab_content" class="tab-content">
                    ...
                    <div id="branch_details" class="py-4 tab-pane fade active show" role="tabpanel" v-show="activeTab === 1">
                        <!-- Name -->
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-semibold text-muted">{{ $t('branch.label.name') }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bold fs-6 text-gray-900">{{ props.branch?.name }}</span>
                            </div>
                        </div>

                        <!-- Branch Manager -->
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-semibold text-muted">{{ $t('branch.label.branchManager') }}</label>
                            <div class="col-lg-8">
                                <!-- <span class="fw-bold fs-6 text-gray-900">{{ props.branch?.id == null ? '' : props.branch?.manager_id }}</span> -->
                                <span v-if="props.branch?.manager_id !== ''">
                                    <span v-for="employee in props?.employees" :key="employee.id">
                                        <span class="fw-bold fs-6 text-gray-900" v-if="employee?.id == props.branch?.manager_id">{{ getFullName(employee) }}</span>
                                        <span v-else>{{ '' }}</span>
                                    </span>
                                </span>
                            </div>
                        ...
                        <!-- Branch Code -->
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-semibold text-muted">{{ $t('branch.label.branchCode') }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bold fs-6 text-gray-900">{{ props.branch?.branch_code }}</span>
                            </div>
                        </div>

                        <!-- Phone Number -->
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-semibold text-muted">{{ $t('branch.label.phoneNumber') }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bold fs-6 text-gray-900">{{ props.branch?.phone_number }}</span>
                            </div>
                        </div>
                        ...
                        <!-- Address -->
                        <div class="row mb-7">
                            <label class="col-lg-4 fw-semibold text-muted">{{ $t('branch.label.address') }}</label>
                            <div class="col-lg-8">
                                <span class="fw-bold fs-6 text-gray-900">{{ props.branch?.address_line_one }} {{ props.branch?.address_line_two }} {{ props.branch?.floor }} {{ props.branch?.city }} {{ props.branch?.state }} {{ props.branch?.zip_code }}</span>
                            </div>
                        </div>
...
watch(activeTab, () => {
    tabTitle.value = activeTab.value === 1 ? t('branch.header.branchDetails') : t('commonComponents.employeeComponent.header.employeeList');
});
